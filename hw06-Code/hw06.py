...
class shiftcipher(methods):
    ...
    def encrypt(self, message):
        ...
        en_message = ''
        for m in message:
            if (ord(m) + self.shift) > ord("z"):
                en_message+=chr((ord(m) + self.shift)-26)
            else:
                en_message+=chr(ord(m) + self.shift)
...
class dictionarycipher(methods):
    ...
    def encrypt(self, message):
        en_message = ''
        for m in message:
            if m in self.dictionary.keys():
                en_message+=self.dictionary[m]
            else:
                en_message+=m
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.deck.cards=newCardlist
