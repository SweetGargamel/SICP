...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        my_lambda_pr=do_lambda_form(Pair(signature.rest,expressions.rest), env)
        env.define(signature.first, my_lambda_pr)
...
def do_if_form(expressions, env):
    ...
    if is_scheme_true(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.rest.first, env,True)#Changed 修改了
    elif len(expressions) == 3:
        return  scheme_eval(expressions.rest.rest.first, env,True)#Changed 修改了
...
def do_and_form(expressions, env):
    ...
    if(expressions is nil):
        ...
        return scheme_eval(expressions.first, env,True)#Changed 修改了
