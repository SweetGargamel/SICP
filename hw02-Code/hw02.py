...
def protected_secret(password, secret, num_attempts):
    ...
    def get_secret(password_attempt):
        if num_attempts == 0:
            ...
            print(secret)
            remained_attempts = num_attempts
        else:
            print("INCORRECT PASSWORD")
            remained_attempts = num_attempts - 1
        return protected_secret(password, secret, remained_attempts)
    ...
    # def get_secret(password_attempt):
    #     # 内部函数用于存储状态
    #     def secret_handler(password_attempt, attempts_left=num_attempts):
    #         if attempts_left <= 0:
    #             print("SECRET LOCKED")
    #             return lambda password_attempt: secret_handler(password_attempt, 0)  # 锁定后保持锁定状态
    #         elif password_attempt == password:
    #             print(secret)
    #             return lambda password_attempt: secret_handler(password_attempt, attempts_left)  # 成功后不减少尝试次数
    #         else:
    #             attempts_left -= 1
    #             if attempts_left > 0:
    #                 print("INCORRECT PASSWORD")
    #             else:
    #                 print("SECRET LOCKED")
    #             return lambda password_attempt: secret_handler(password_attempt, attempts_left)  # 返回更新的状态函数

    #     # 初次调用时从 num_attempts 开始
    #     return secret_handler(password_attempt)
